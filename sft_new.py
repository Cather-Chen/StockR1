# 0408: now correctly use forecast_hint.
import os
...
def collect_phase_params(model: nn.Module, default_trainable: Dict[str, bool]):
    hint_params = []
    main_params = []
    for name, p in model.named_parameters():
        if not default_trainable.get(name, False):
            continue
        if name.startswith("hint_conditioner."):
            hint_params.append(p)
        else:
            main_params.append(p)
    return hint_params, main_params
...
def build_argparser():
    ...
    parser.add_argument("--special_tokens", nargs="*", default=["<answer>", "</answer>", "<forecast_ts>", "</forecast_ts>", 
                                                                "<forecast_router>", "<forecast_hint>", "</forecast_hint>"])
    ...
    parser.add_argument("--task", type=str, default="sft_new", choices=["sft_new"], help="Pass through to StockDataLoader")
    parser.add_argument("--ts_soft_tokens", action="store_true", help="Enable soft tokens for the TS encoder.")
    ...
    # optimization
    parser.add_argument("--sft_epochs", type=int, default=5, help="Phase-2 epochs: joint SFT + TS loss training.")
    parser.add_argument("--hint_only_epochs", type=int, default=100, help="Phase-1 epochs: train only ForecastHintConditioner using TS loss.")
    ...
    parser.add_argument("--eval_every_steps", type=int, default=1000, help="If >0, run eval every N optimizer steps; otherwise eval each epoch.")
    parser.add_argument("--run_name", type=str, default=None)
    parser.add_argument("--group_name", type=str, default="sft")
...
def main():
    ...
    print(f" 🚀 Launch with args: "
          ...
          f"accum={args.grad_accum_steps}, sft_epochs={args.sft_epochs}, hint_only_epochs={args.hint_only_epochs}, lora={args.lora_enable}")
    ...
    train_loader = loader.create_loader(split="train", use_distributed_sampler=False)
    val_loader   = loader.create_loader(split="test", use_distributed_sampler=False)
    ...
    default_trainable = {name: p.requires_grad for name, p in model.named_parameters()}
    total_epochs = args.sft_epochs + args.hint_only_epochs
    if total_epochs <= 0:
        raise ValueError("total epochs must be > 0. Check --sft_epochs and --hint_only_epochs.")


    # optimizer / scheduler
    optim = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    total_train_steps = total_epochs * math.ceil(len(train_loader) / max(1, 1))
    ...
    current_phase = None
    unwrap_model = accelerator.unwrap_model(model)
    hint_phase_params, main_phase_params = collect_phase_params(unwrap_model, default_trainable)
    ...
    for epoch in range(1, total_epochs + 1):
        phase = "hint_ts_only" if epoch > args.sft_epochs else "sft_plus_ts"
        if phase != current_phase:
            current_phase = phase
            if accelerator.is_main_process:
                total = count_all_params(unwrap_model)
                active = sum(p.numel() for p in (hint_phase_params if phase == "hint_ts_only" else (hint_phase_params + main_phase_params)))
                accelerator.print(f" 🔧 Switch training phase -> {phase} | active={active/1e6:.2f}M / total={total/1e6:.2f}M")
        ...
        accelerator.print(f" ▶️ Epoch {epoch}/{total_epochs} started. phase={phase}")
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            if phase == "hint_ts_only":
                batch["ts_only"] = True

            with accelerator.accumulate(model):
                with accelerator.autocast():
                    out = model(batch)
                    loss = out["ts_loss"] if phase == "hint_ts_only" else out["loss"]
                accelerator.backward(loss)
                if phase == "hint_ts_only":
                    clear_grads(main_phase_params)
            ...
            if accelerator.sync_gradients and (global_step % args.log_interval == 0):
                ...
                accelerator.print(f" step {step}/{len(train_loader)} "
                                  f"(global {global_step}) | phase={phase} | loss={loss_val:.4f} "
            ...
            if accelerator.sync_gradients and args.eval_every_steps > 0 and (global_step % args.eval_every_steps == 0):
                accelerator.wait_for_everyone()
