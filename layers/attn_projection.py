...
class RotaryProjection(Projection):
    ...
    def forward(self, x, seq_id):
        ...
        # Ensure cos and sin are on the same device as seq_id
...
class QueryKeyProjection(nn.Module):
    def __init__(self, dim: int, num_heads: int, proj_layer, kwargs=None, partial_factor=None):
        '''
        dim: the dimension of the query and key
        num_heads: the number of heads
        proj_layer: the layer to project the query and key
        kwargs: the keyword arguments to pass to the proj_layer
        partial_factor: the partial factor of the query and key
        '''
        super().__init__()
